# accounts/tests.py — Tests complets pour le module accounts (CORRIGÉ)
"""
...
class EntrepriseModelTest(TestCase):
    ...
    def test_slug_unique_with_counter(self):
        ...
        # ✅ CORRECTION: nom doit être unique, donc noms différents
        e1 = Entreprise.objects.create(nom="CHU Angre A")
        e2 = Entreprise.objects.create(nom="CHU Angre B")
        self.assertEqual(e1.slug, "chu-angre-a")
        self.assertEqual(e2.slug, "chu-angre-b")
...
class RoleEntrepriseModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.entreprise = Entreprise.objects.create(nom="Test")
        # ✅ CORRECTION: nom unique pour éviter conflit avec setUp précédent
        self.group = Group.objects.create(name="AdminRole@Test")
    ...
    def test_creation(self):
        ...
        self.assertEqual(str(re), "AdminRole@Test — Test")

    def test_nom_affiche(self):
        """nom_affiche doit retourner le nom sans le suffixe @entreprise."""
        from django.contrib.auth.models import Group
        # ✅ CORRECTION: nom unique
        g = Group.objects.create(name="Manager@Test")
        re = RoleEntreprise.objects.create(groupe=g, entreprise=self.entreprise)
        self.assertEqual(re.nom_affiche, "Manager")


class EntrepriseSlugEdgeCasesTest(TestCase):
    """Tests edge cases pour le slug d'Entreprise."""

    def test_empty_nom_saved_as_empty(self):
        """Un nom vide est accepté (pas de validation côté modèle)."""
        # ✅ CORRECTION: Entreprise n'a pas de validation sur nom vide
        # On teste juste que le slug est vide aussi
        e = Entreprise.objects.create(nom="")
    ...
    def test_multiple_entreprises_same_base_name(self):
        ...
        # ✅ CORRECTION: noms uniques car Entreprise.nom est unique
        names = ["Clinique A", "Clinique B", "Clinique C"]
        slugs = []
        for name in names:
        ...
        self.assertEqual(slugs[0], "clinique-a")
        self.assertEqual(slugs[1], "clinique-b")
        self.assertEqual(slugs[2], "clinique-c")