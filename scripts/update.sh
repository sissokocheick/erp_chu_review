...
cd "$APP_DIR" || { echo -e "Erreur: Le dossier $APP_DIR n'existe pas."; exit 1; }

# 2. Récupération du nouveau code depuis GitHub
echo -e "\n[1/4] Récupération des dernières modifications (git pull)..."
sudo -u "$APP_USER" git pull origin main || { echo -e "Erreur lors du git pull."; exit 1; }

# 3. Application des migrations et fichiers statiques
echo -e "\n[2/4] Application des migrations base de données..."
sudo -u "$APP_USER" bash -c 'set -a; source .env; set +a; venv/bin/python manage.py migrate --noinput' || { echo -e "Erreur lors des migrations."; exit 1; }

echo -e "\n[3/4] Collecte des fichiers statiques..."
sudo -u "$APP_USER" bash -c 'set -a; source .env; set +a; venv/bin/python manage.py collectstatic --noinput' || { echo -e "Erreur lors du collectstatic."; exit 1; }
